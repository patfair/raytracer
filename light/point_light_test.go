...
func TestNewPointLight(t *testing.T) {
	light, err := NewPointLight(geometry.Point{1, 2, 3}, shading.Color{0.1, 0.2, 0.3}, 254, 2.54, 3)
	assert.Nil(t, err)
...
func TestNewPointLightInvalid(t *testing.T) {
	_, err := NewPointLight(geometry.Point{1, 2, 3}, shading.Color{0.1, 0.2, 0.3}, -1, 2.54, 3)
	if assert.NotNil(t, err) {
		assert.Contains(t, err.Error(), "must be positive")
	}

	_, err = NewPointLight(geometry.Point{1, 2, 3}, shading.Color{0.1, 0.2, 0.3}, 0, 2.54, 3)
	if assert.NotNil(t, err) {
		assert.Contains(t, err.Error(), "must be positive")
	}

	_, err = NewPointLight(geometry.Point{1, 2, 3}, shading.Color{0.1, 0.2, 0.3}, 1, -0.1, 3)
	if assert.NotNil(t, err) {
		assert.Contains(t, err.Error(), "must be non-negative")
	}

	_, err = NewPointLight(geometry.Point{1, 2, 3}, shading.Color{0.1, 0.2, 0.3}, 1, 0, 0)
	if assert.NotNil(t, err) {
		assert.Contains(t, err.Error(), "must be at least 1")
	}

	_, err = NewPointLight(geometry.Point{1, 2, 3}, shading.Color{0.1, 0.2, 0.3}, 1, 0, 3)
	if assert.NotNil(t, err) {
...
func TestPointLight_Direction(t *testing.T) {
	...
	light, _ = NewPointLight(geometry.Point{0, 0, 1}, shading.Color{1, 1, 1}, 1, 0.1, 10)
	...
	light, _ = NewPointLight(geometry.Point{0, 0, 1}, shading.Color{1, 1, 1}, 1, 0.001, 2)
...
func TestPointLight_IsBlockedByIntersection(t *testing.T) {
	...
	light, _ := NewPointLight(geometry.Point{1, -1, 5}, shading.Color{1, 1, 1}, 1, 0, 1)
