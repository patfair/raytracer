...
func TestPointLightDirection(t *testing.T) {
	...
	assert.Equal(t, Vector{0, 0, -1}, light.Direction(Point{0, 0, 0}, 0, 1))
	assert.Equal(t, Vector{0, 0, -1}, light.Direction(Point{0, 0, 0}, 1, 1))
	assert.Equal(t, Vector{0, 0, -1}, light.Direction(Point{0, 0, 0}, 2, 1))

	light = PointLight{point: Point{0, 0, 1}, radius: 0.1}
	direction := Vector{0, 0, -1}

	for i := 0; i < 10; i++ {
		newDirection := light.Direction(Point{0, 0, 0}, i, 10)
